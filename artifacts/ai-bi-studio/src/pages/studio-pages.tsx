    ...
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1.35fr)_minmax(300px,.65fr)]">
      <section className="animate-enter-up delay-1">
        <form onSubmit={submitQuestion} className="relative overflow-hidden rounded-sm border border-primary/30 bg-card shadow-sm">
          ...
          <div className="flex flex-col gap-3 border-t border-border bg-muted/25 px-5 py-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex flex-wrap items-center gap-2"><label className="flex items-center gap-2 rounded-sm border border-border bg-card px-2.5 py-1.5 text-[11px] text-muted-foreground"><Database size={12} className="text-primary" /><select defaultValue={activeConnection?.id ?? ''} disabled={!activeConnection} className="max-w-[150px] bg-transparent font-medium text-foreground outline-none" data-testid="select-connection"><option value={activeConnection?.id}>{activeConnection?.name ?? 'No connection'}</option></select></label><label className="flex items-center gap-2 rounded-sm border border-border bg-card px-2.5 py-1.5 text-[11px] text-muted-foreground"><Terminal size={12} /><select value={dialect} onChange={(event) => setDialect(event.target.value as QueryRunInputDialect)} className="bg-transparent font-medium text-foreground outline-none" data-testid="select-dialect">{dialectOptions.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label><button type="button" onClick={() => setMode(mode === 'analyst' ? 'sql_only' : 'analyst')} className={`rounded-sm border px-2.5 py-1.5 text-[11px] font-medium transition-colors ${mode === 'analyst' ? 'border-primary/25 bg-primary/10 text-primary' : 'border-border bg-card text-muted-foreground'}`} data-testid="button-toggle-mode">{mode === 'analyst' ? 'Analyst mode' : 'SQL only'}</button></div><button type="submit" disabled={!question.trim() || !activeConnection || createRun.isPending} className="inline-flex items-center justify-center gap-2 rounded-sm bg-primary px-4 py-2.5 text-xs font-bold text-primary-foreground transition-transform hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-45" data-testid="button-run-query">{createRun.isPending ? <Loader2 size={14} className="animate-spin" /> : <Play size={14} fill="currentColor" />}{createRun.isPending ? 'Orchestrating…' : 'Run analysis'}</button></div>
